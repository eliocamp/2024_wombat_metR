...
title-slide-attributes:
  data-background-image: "img/title_crop_blur.png"
...
# Reading data {background-image="img/mosaico.png" background-position="right" background-size="contain" background-opacity="1"}
...
# Plotting data {background-image="img/papel_mask.png" background-position="right" background-size="contain" background-opacity="1"}
...
# Transforming data {background-image="img/pp_blur.png" background-position="right" background-size="contain" background-opacity="1"}
...
I picked 1998 because it was a special year with a monster El Niño. 
...
```{r}
cut(sst_eofs, 1:2)$left |> 
  ggplot(aes(time, sst_a)) +
  geom_line(aes(color = PC))
```


# Analyse data {background-image="img/bauhaus.png" background-position="right" background-size="contain" background-opacity="1"}
...
Reshape into long and compute anomalies. 
...
regression <- for_regression[, metR::FitLm(value_a, PC1, PC2),
...
#| code-line-numbers: "|4,5,6|6"
regression_spring |> 
  ggplot(aes(lon, lat)) +
  geom_contour_fill(aes(z = pp)) +
  metR::geom_contour2(aes(z = z/9.8, 
...
[eliocamp.github.io/slides/wombat2024](eliocamp.github.io/slides/wombat2024)
